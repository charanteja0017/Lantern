...
class BrowserTabManager:
    ...
    def _session_id_for_agent(self, agent_id: str) -> str:
        """Resolve the logical session ID the agent should report under.

        The phase is read from ``STRIX_AGENT_PHASE`` which the agent graph
        sets when spawning workers. If unset we fall back to an agent-scoped
        session so telemetry stays useful even without phase metadata.
        """
        phase = os.getenv("STRIX_AGENT_PHASE") or os.getenv("STRIX_PHASE")
        return get_session_registry().resolve_session(phase=phase, agent_id=agent_id)
    ...
    def launch_browser(self, url: str | None = None) -> dict[str, Any]:
        with self._lock:
            ...
            try:
                ...
                registry.record_failed(session_id, agent_id=agent_id, reason=f"launch:{e}")
                raise RuntimeError(f"Failed to launch browser: {e}") from e
            registry.record_created(session_id, agent_id=agent_id)
            return result
    ...
    def close_browser(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            try:
                ...
                get_session_registry().record_failed(
                    session_id, agent_id=agent_id, reason=f"close:{e}"
                )
                raise RuntimeError(f"Failed to close browser: {e}") from e
            get_session_registry().record_recycled(
                session_id, agent_id=agent_id, reason="explicit_close"
            )
            return {
    ...
    def cleanup_agent(self, agent_id: str) -> None:
        ...
        if browser:
            ...
            with contextlib.suppress(Exception):
                get_session_registry().record_recycled(
                    session_id, agent_id=agent_id, reason="cleanup_agent"
                )
