...
_racer_lanes: dict[str, dict[str, Any]] = {}
...
@register_tool(sandbox_execution=False)
def create_racer_lane(
    agent_state: Any,
    lane_name: str,
    task: str,
    max_agents: int = 3,
) -> dict[str, Any]:
    lane_id = f"lane_{len(_racer_lanes) + 1}"
    _racer_lanes[lane_id] = {
        "lane_id": lane_id,
        "name": lane_name,
        "task": task,
        "max_agents": max_agents,
        "owner_agent_id": agent_state.agent_id,
        "agents": [],
        "status": "ready",
    }
    return {"success": True, "lane": _racer_lanes[lane_id]}


@register_tool(sandbox_execution=False)
def list_racer_lanes() -> dict[str, Any]:
    return {"lanes": list(_racer_lanes.values()), "count": len(_racer_lanes)}
