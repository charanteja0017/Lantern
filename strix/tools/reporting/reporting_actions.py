...
def _validate_required_fields(**kwargs: str | None) -> list[str]:
    ...
    # Run the canonical finding-evidence validator in addition to the
    # lightweight presence check above. This enforces the same rules the
    # report-assembly stage uses so a finding can't sneak past one layer
    # only to be rejected by the next.
    try:
        from strix.core.validation.exploit_queue_schema import validate_finding_evidence

        # The tool signature doesn't carry a finding ID yet; synthesize a
        # stable one from the title for duplicate-detection downstream.
        candidate = {
            "id": kwargs.get("id") or f"candidate:{(kwargs.get('title') or '').strip()[:64]}",
            ...
            "poc_script_code": kwargs.get("poc_script_code"),
            "remediation_steps": kwargs.get("remediation_steps"),
            "blocked_by_control": bool(kwargs.get("blocked_by_control")),
            "blocked_by_control_reason": kwargs.get("blocked_by_control_reason"),
        }
        for err in validate_finding_evidence(candidate):
            # De-dup against the basic messages we already added.
            msg = err.message
