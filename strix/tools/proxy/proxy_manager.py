...
class ProxyManager:
    def __init__(self, auth_token: str | None = None):
        ...
        # Proxy traffic may target arbitrary hosts; TLS verification should stay enabled.
        # If you really need to disable verification (e.g. intercepting with a custom CA
        # not installed on the host), set CAIDO_INSECURE_SSL=true explicitly.
        self.verify_tls = os.getenv("CAIDO_INSECURE_SSL", "").lower() not in {"1", "true", "yes"}
    ...
    def view_sitemap_entry(self, entry_id: str) -> dict[str, Any]:
        try:
            ...
            return {"entry": cleaned} if cleaned else {"error": "Failed to process sitemap entry"}
