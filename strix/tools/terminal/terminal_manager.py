...
class TerminalManager:
    ...
    def execute_command(
        ...
        try:
            ...
            logger.warning("terminal execute_command failed: %s", e)
            return {
        ...
        with self._lock:
            if terminal_id not in sessions:
                try:
                    ...
                    raise RuntimeError(
                        "Terminal session unavailable — tmux/libtmux may be missing or "
                        "unsupported in this environment (dashboard API containers "
                        "typically do not embed an interactive tmux backend)."
                    ) from exc
