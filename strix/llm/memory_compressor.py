...
def _summarize_messages(
    ...
    try:
        ...
        # Role router: memory summarization is cheap and fits a small-context
        # model well, so we tag this call site so operators can route it
        # away from their expensive reasoning model.
        from strix.llm.router import get_router  # local import to avoid cycles

        completion_args = get_router().prepare_completion_args("memory", completion_args)
