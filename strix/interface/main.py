...
async def _hydrate_router_if_possible() -> None:
    """Best-effort hydrate the LLM role router from Postgres at CLI start.

    The subprocess shares the ``strix_llm_routes`` table with the API so
    per-run and global overrides apply here too. Failures are silent — a
    missing DB simply means we fall back to env vars.
    """
    try:
...
async def warm_up_llm() -> None:
    ...
    try:
        ...
        # Route the connectivity warm-up through the "default" role so the
        # check matches what the rest of the run will actually use when no
        # per-role override is set.
        from strix.llm.router import get_router

        completion_kwargs = get_router().prepare_completion_args("default", completion_kwargs)
...
Examples:
  ...
  strix --target https://app.com --instruction-file /path/to/detailed_instructions.md
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Override the auto-generated run name. Output is written to "
            "'strix_runs/<run-name>/' instead of the default '<slug>_<hex>' "
            "naming. Used by the Strix API to align CLI outputs with the "
            "dashboard's run identifiers."
        ),
    )
...
def main() -> None:
    ...
    asyncio.run(_hydrate_router_if_possible())
    asyncio.run(warm_up_llm())

    persist_config()

    # Honour an explicit --run-name override (used by the Strix API to keep
    # CLI output in the same directory as the API's per-run metadata/pidfile).
    # The override is restricted to filesystem-safe characters; anything else
    # falls back to the auto-generated name for safety.
    override = getattr(args, "run_name", None)
    if override and re.fullmatch(r"[A-Za-z0-9._\-]{1,64}", override):
        args.run_name = override
    else:
