...
class BaseAgent(metaclass=AgentMeta):
    ...
    async def _process_iteration(self, tracer: Optional["Tracer"]) -> bool | None:
        ...
        # --- Swarm blackboard: inject current top findings into context -----
        # Best-effort: if Postgres is unavailable the blackboard returns [].
        try:
            from strix.nova.blackboard import get_blackboard

            board = get_blackboard()
            run_id = tracer.run_id if tracer else ""
            findings = await board.list_findings(run_id=run_id, limit=12, offset=0)
            if findings:
                lines = []
                for f in findings[:12]:
                    eff = f.effective_pheromone
                    lines.append(
                        f"- kind={f.kind} eff={eff:.3f} sev={f.severity:.2f} payload={f.payload}"
                    )
                injected = (
                    "<swarm_blackboard>\n"
                    "Shared findings (highest effective pheromone first). Use these to decide next actions.\n"
                    + "\n".join(lines)
                    + "\n</swarm_blackboard>"
                )
                self.state.add_message("user", injected)
        except Exception:
            pass
    ...
    async def _execute_actions(self, actions: list[Any], tracer: Optional["Tracer"]) -> bool:
        ...
        Enforces ``LLMConfig.per_turn_iterations`` (default 25): if the LLM
        emits more tool calls in a single turn than the cap allows, the
        trailing ones are dropped and a corrective observation is appended so
        the model sees the truncation on its next turn.
        """
        per_turn_cap = getattr(self.llm_config, "per_turn_iterations", 0)
        truncated_count = 0
        if per_turn_cap and len(actions) > per_turn_cap:
            truncated_count = len(actions) - per_turn_cap
            actions = actions[:per_turn_cap]
        ...
        if truncated_count > 0:
            truncation_note = (
                f"Per-turn tool cap reached: you requested "
                f"{truncated_count + per_turn_cap} tool calls in a single "
                f"turn but the limit is {per_turn_cap}. The last "
                f"{truncated_count} call(s) were dropped. Issue them again "
                "on a subsequent turn if still needed, or rethink the "
                "approach if this was unintentional."
            )
            self.state.add_error(truncation_note)
        ...
        if truncated_count > 0:
            conversation_history.append({"role": "user", "content": truncation_note})

        self.state.messages = conversation_history

        # --- Swarm blackboard: record tool results (best-effort) ------------
        try:
            from strix.nova.blackboard import get_blackboard

            board = get_blackboard()
            run_id = tracer.run_id if tracer else ""
            # Extract the latest tool_result messages and record a compact summary.
            tail = (
                conversation_history[-8:] if len(conversation_history) > 8 else conversation_history
            )
            for msg in tail:
                ...
                if not content.strip():
                    continue
                await board.write_finding(
                    run_id=run_id,
                    kind="ToolResult",
                    payload={"agent_id": self.state.agent_id},
                    evidence={"tool_output": content[:2000]},
                    confidence=0.6,
                    severity=0.0,
                    pheromone=0.15,
