...
def set_global_tracer(tracer: "Tracer") -> None:
    ...
    # Propagate the run_id into the LLM router's ambient scope so per-run
    # overrides kick in automatically without threading run_id through
    # every call site. The router import is local to avoid a hard cycle
    # between telemetry and the LLM layer.
    try:
        from strix.llm.router import get_router

        run_id = getattr(tracer, "run_id", None)
        if run_id:
            get_router().set_ambient_scope(run_id=run_id)
    except Exception:
        pass
