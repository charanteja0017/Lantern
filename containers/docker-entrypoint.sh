...
echo "Starting side-channel daemons (shellinabox + noVNC/VNC)..."
if command -v supervisord >/dev/null 2>&1; then
  SHELLINABOX_PORT="${SHELLINABOX_PORT:-4200}" \
  NOVNC_PORT="${NOVNC_PORT:-6901}" \
  VNC_PORT="${VNC_PORT:-5901}" \
  supervisord -c /etc/supervisor/conf.d/supervisord.conf >/tmp/supervisord-start.log 2>&1 &
  sleep 1
else
  echo "supervisord not found; skipping side-channel daemons"
fi
