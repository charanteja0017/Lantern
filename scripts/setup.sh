#!/usr/bin/env bash
# Lantern — one-shot installer for the full backend + frontend stack.
#
...
log()  { printf "%s[lantern]%s %s\n" "${CYAN}"   "${NC}" "$*"; }
warn() { printf "%s[lantern]%s %s\n" "${YELLOW}" "${NC}" "$*"; }
ok()   { printf "%s[lantern]%s %s\n" "${GREEN}"  "${NC}" "$*"; }
die()  { printf "%s[lantern]%s %s\n" "${RED}"    "${NC}" "$*" >&2; exit 1; }
...
This script must be run from inside a full Lantern checkout."
fi

HTTP_PORT="${LANTERN_HTTP_PORT:-80}"
...
confirm_or_abort() {
    ...
    printf "%s[lantern]%s %s [y/N]: " "${YELLOW}" "${NC}" "$prompt"
    local reply
    read -r reply || reply=""
...
preflight_ports() {
    ...
    if $SUDO ss -ltn 2>/dev/null | awk '{print $4}' | grep -Eq ":${port}$"; then
        ...
        warn "Either stop the conflicting service or set LANTERN_HTTP_PORT to a free port."
    fi
}
        ...
        if [[ -n "$pg_container" ]]; then
            ...
            run_shell "set -euo pipefail
                ...
                $SUDO docker compose \
                    ...
                    printf '%s[lantern]%s pg_dumpall skipped\n' '${YELLOW}' '${NC}'"
        fi
    fi
...
write_env() {
    ...
    if [[ -f "$env_file" ]]; then
        ...
        upsert_env_var "$env_file" "LANTERN_HTTP_PORT" "$HTTP_PORT"
        return 0
    fi
...
# Lantern deployment config (generated by scripts/setup.sh on $(date -u +%FT%TZ)).
...
# Lantern ships its own Kali-based sandbox image that bundles Burp CE,
...
# The web dashboard is one half of Lantern; the other half is the `strix`
...
main() {
    log "Lantern installer starting ..."
    preflight_summary
    preflight_ports
    install_prereqs
    backup_state
