...
The Strix monorepo ships two workflows under `.github/workflows/` that
execute the same checks this page documents:

| Workflow | Trigger | Jobs |
|----------|---------|------|
| `ci.yml` | push + PR to `main`/`master` | ruff lint, ruff format check, mypy, pytest, frontend lint + type-check + build, `docker compose config`, `scripts/setup.sh --dry-run` |
| `release.yml` | tags matching `v*.*.*` | builds wheel/sdist, publishes container to GHCR, creates a GitHub release with generated notes |

Keep these YAML files in sync with any updates you make to this page — the
workflows are the executable source of truth. If you add a new pipeline
step in docs, add it to `ci.yml` first so the docs never drift ahead of
the actual pipeline.
